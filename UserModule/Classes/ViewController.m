...
@interface ViewController ()<WXRespManagerDelegate>
        ...
        [[HttpClient shareInstance] LoginGuest:^(id responseObject, NSError *error) {
            if (!error) {
                ...
                if (responseErrcode == 0 && responseObject) {
                    _isSupportMobileLogin = NO;
                } else if (responseErrcode ==502){
                    _isSupportMobileLogin = YES;
                } else {
                    _isSupportMobileLogin = NO;
                }
            } else {
...
- (void)userLogin {
    BOOL isInstall = [[UMSocialWechatHandler defaultManager] umSocial_isInstall];
...
- (void)showLoginFailWithNetWork {
    UIColor *color = [UIColor colorWithHexString:@"#285B9A"];
...
- (UIButton *)loginButton{
    if (!_loginButton) {
        ...
        _loginButton.backgroundColor = [UIColor colorWithHexString:@"#25ac1d"];
        [_loginButton setTitle:@"微信登录" forState:UIControlStateNormal];
        [_loginButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _loginButton.titleLabel.font = [UIFont systemFontOfSize:16];
        _loginButton.layer.shadowColor = [UIColor colorWithHexString:@"#040404"].CGColor;
...
- (UIButton *)protocolBtn{
    if (!_protocolBtn) {
        ...
        [_protocolBtn setTitleColor:[UIColor colorWithHexString:@"#4b6580"] forState:UIControlStateNormal];
